...
|--------------|-------------------------|----------------------------------------------------------------|
| char(m)      | m = délka řetězce 0-255 | Pokud se vložený řetězec kratší, doplní se volná místa mezerami |
...
| binary (m)   | m = délka řetězce       | Pokud se vložený řetězec je kratší, doplní se volná místa mezerami |
...
*Datové typy Binary slouží pro ukládání hashů, kontrolních součtů a dalších binárních dat
...
**FOREIGN KEY** – předchází zničení propojenosti tabulek.
...
zde ještě zmínit PostgreSQL, který k tomu používá samotný datový typ serial. Zpět do MySQL, zde se AUTO_INCREMENT používá
...
indexy používají tvz. B-stromy, které jsou postavené speciálně pro tyto operace.
...
Unikátní index – zajišťuje, že dva řádky nebudou mít stejnou hodnotu.
