...
- příkaz který vrací tabulku s krátkým popisem a popřípadě i syntaxi dotazovaného příkazu/funkce
...
- vybere všechny výsledky z „levé” tabulky (t1) a spojí všechny, které hodnotu z druhé tabulky mají. Pro ty řádky, které nemají hodnotu v pravé tabulce, se hodnota nastaví NULL.
...
```sql
SELECT zam.id zam.name FROM zaměstnanci as zam 
WHERE zam.id IN ( SELECT zam.id FROM zaměstnanci as zam WHERE plat > 50000);
```
...
- View je virtuální tabulka která je pouze struktura (query) a neobsahuje data. Umožňuje vidět uživateli data z 1 i více tabulek v právě této najednou. Protože view je pouze struktura nemusí využívat joiny v klauzuli FROM označovat více tabulek najednou.
