...
- DBMS (Database Management System) je systém, který umožňuje uživatelům vytvářet, udržovat a manipulovat s databázemi.
...
- umožňuje ukládat strukturovaná data jako v relačních DB, ale také zahrnuje objektové funkce a datové typy
...
- Matematické struktury skládající se z uzlů spojených hranami, které mohou reprezentovat různé vztahy mezi daty
...
- Vytvoření uživatele s použitím autentizačního pluginu pro zahashovaní hesla, heslo po 180 dnech vyprší, má 3 pokusy na přihlášení, jinak se účet zablokuje na 2 dny

```sql
CREATE USER 'Vojta'@'localhost' IDENTIFIED WITH caching_sha2_password BY 'tajneheslo' PASSWORD EXPIRE INTERVAL 180 DAY FAILED_LOGIN_ATTEMPTS 3 PASSWORD_LOCK_TIME 2;
```
...
- Skupina oprávnění, které mohou být uděleny uživatelům, výhoda nemusí se nastavovat každý uživatel a stačí jemu přidělí role
...
- **C - Consistency (Konzistence)**
    - Musí zajistit, že data budou před provedením transakce v konzistentním stavu. To znamená, že transakce musí respektovat všechna pravidla integrity dat
- **I - Isolation (Izolace)**
