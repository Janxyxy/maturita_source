...
- GUA
  - Adresy pro veřejné použití
    - Skládá se ze 3 částí – prefix se dělí na Global Routing Prefix a Subnet ID
       - Global Routing Prefix
         - Dáno poskytovatelem
         - Typicky 48 (větší prefix by znamenal menší Subnet ID)
         - 2001:db8:acad::/48
       - Subnet ID
         - Speciální vlastnost IPv6. Verze 4 si pro subnety musela půjčovat bity z části pro hosty. IPv6 na to myslela předem a do hostující části nezasahuje.
         - Typicky 16 bitů
       - Interface ID
         - Část adresy pro hosty

| Adresa                                | Global Routing Prefix (48 bitů) | Subnet ID (16 bitů) | Interface ID (64 bitů) |
|---------------------------------------|---------------------------------|---------------------|------------------------|
| 2001:db8:acad:xxxx:yyyy:yyyy:yyyy:yyy | 2001:db8:acad                   | :xxxx               | :yyyy:yyyy:yyyy:yyy    |


- LLA
-  Umožňuje komunikaci na své síti (subnetu) à neprojde tedy přes router
- LLA je na rozdíl od GUA povinná (musí být nastavená, pokud máme aktivovaný IPv6 routing) – pokud není nastavená manuálně, host si ji sám vytvoří.
- Fe80::/10
- Statická (manuálně nastavená) X dynamická (host si vytvoří sám pomocí EUI)
- Pro vygenerování 64 bitů pro Interface ID
  - Vezme MAC adresu (48 bitů)
  - Rozdělí v polovině (přesně tak, jak je MAC rozdělená na výrobce (Organizationally
    Unique Identifier) a device ID
  - Do rozdělené poloviny vloží 16 bitů – FF FE
  - Změní hodnotu sedmého bitu (0 à 1 a obráceně)
  - FC:99:47:75:CE:E0 -> F `C` 99 47 `FF FE` 75 CE E0 -> F`E`99:47`FF:FE`75:CEE0
### Cíle
