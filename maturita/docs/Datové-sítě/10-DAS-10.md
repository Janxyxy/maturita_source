...
- Stateless
  - nikdo nekontroluje zda je adresa již použita
  - klient si sám vybere adresu
  - SLAAC
    - Router posílá Router Advertisement (RA) messages
    - To informuje o adrese sítě, prefixu a default gateway
    - Host si z těch informací sám vygeneruje adresu
  - SLAAC + DHCPv6
    - Router RA message poskytne stejnou informaci jako SLAAC + kontakt na DHCP server, který poskytne zbytek
    - Host si sám generuje adresu na základě poskytnutých info
    
- Stateful
  - DHCPv6 (Stateful DHCPv6)
    - Router pošle RA, která informuje o default gateway a oznámí, že pro další informace musí kontaktovat DHCP server
    - Host získá DG z RA a zbytek informací z DHCP serveru
