...
Veškeré aktuálně provedené příkazy jsou ukládány pouze do paměti RAM (souboru running-config). Po restartu zařízení jsou tyto příkazy ztraceny. Je tedy důležité používat příkaz copy pro zapsání do startup konfigurace.
